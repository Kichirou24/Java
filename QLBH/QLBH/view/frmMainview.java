...
public class frmMainview extends JFrame {
	...
	public void insert()
	{
		try {
			...
			Integer soLuong = 0;
			if (isValidSoLuong(s) == true)
				soLuong = Integer.parseInt(s);
			else
			{
				JOptionPane.showMessageDialog(null, "Nhap so luong dung dinh dang");
				return;
			}
	...
	public Boolean checkSoLuong(String maHang, Integer soluongmua)
	{
		for (Hangbean h : ds)
		{
			if (h.getMaHang().trim().equals(maHang.trim()))
				...
				{
					h.setSoLuong(h.getSoLuong() - soluongmua); // tru trong kho hang
					return true;
				}
		}
		return true;
	...
	public Boolean isValidSoLuong(String s)
	{
	...
	public void Add() throws Exception
	{
		try {
			...
			String s = JOptionPane.showInputDialog("Nhap so luong mua");
			...
			for (BanHangbean b : dsBH) {
				if (b.getMaHang().trim().equals(maHang.trim()))
				{
					...
					{
						b.setSoLuongMua(soluongmua + b.getSoLuongMua()); // cong trong hoa don
						bhbo.add(bh);
						updateTable(ds);
						updateTable2(dsBH);
					}
			...
			JOptionPane.showMessageDialog(null, "Vui long nhap so luong muon mua");
		}
	}
